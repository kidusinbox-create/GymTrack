...
const MUSCLE_MAP = {
  chest:       { label: 'Chest' },
  front_delts: { label: 'Front Delts' },
  side_delts:  { label: 'Side Delts' },
  rear_delts:  { label: 'Rear Delts' },
  biceps:      { label: 'Biceps' },
...
function BodyFront({ muscleData }) {
  ...
  return (
    <svg viewBox="0 0 120 260" style={{ width: '100%', height: '100%' }}>
      ...
      <path d="M35 42 Q25 50 22 80 L22 130 Q22 135 28 135 L92 135 Q98 135 98 130 L98 80 Q95 50 85 42 Z" fill="#222" stroke="#333" strokeWidth="0.5" />
...
function BodyBack({ muscleData }) {
  ...
  return (
    <svg viewBox="0 0 120 260" style={{ width: '100%', height: '100%' }}>
      ...
      <path d="M40 110 Q48 108 60 108 Q72 108 80 110 L80 132 Q72 135 60 135 Q48 135 40 132 Z" style={s('lower_back')} />
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', gap: 6 }}>
      ...
      <div style={{ flex: 1, display: 'flex', gap: 6, minHeight: 0 }}>
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
          <p style={{ fontSize: 8, color: 'var(--text-muted)', textAlign: 'center', marginBottom: 2, textTransform: 'uppercase', flexShrink: 0 }}>Front</p>
          <div style={{ flex: 1, minHeight: 0 }}>
            <BodyFront muscleData={muscleData} />
          </div>
        </div>
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
          <p style={{ fontSize: 8, color: 'var(--text-muted)', textAlign: 'center', marginBottom: 2, textTransform: 'uppercase', flexShrink: 0 }}>Back</p>
          <div style={{ flex: 1, minHeight: 0 }}>
      ...
      {activeMusclePairs.length > 0 && (
        <div style={{ borderTop: '1px solid var(--grid-line)', paddingTop: 6, flexShrink: 0 }}>
            ...
            {activeMusclePairs.map(([muscle, intensity]) => {
              ...
              return (
                <div key={muscle} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <p style={{ fontSize: 10, color: 'var(--text-secondary)', width: 70, flexShrink: 0 }}>
                    {info?.label ?? muscle}
                  </p>
                  ...
                  <p style={{ fontSize: 9, color: 'var(--text-secondary)', width: 28, textAlign: 'right' }}>
